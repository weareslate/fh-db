...
function getDitchHandle(cb) {
  ...
  console.log("[fh-db/localdb.js:local_db:30] in getDitchHandle");
  var my_db_config = {
    database: {
  ...
  if (mongoConnectionURL) {
    ...
    console.log("[fh-db/localdb.js:local_db:30] try getparseMongoConnectionURL ");

    try {
      my_db_config = parseMongoConnectionURL(mongoConnectionURL);
    }
    catch (e) {
      var errMessage = new Error(`Error parsing database connection string - verify format. ${e}`);
  ...
  if (!ditch) {
    ...
    var ditcher = new fhditcher.Ditcher(my_db_config, my_db_logger, versString, function () {
      console.log("[fh-db/localdb.js:local_db:45] ditcher initalised");
...
var local_db = function (params, cb) {
  ...
  console.log("[fh-db/localdb.js:local_db:64] DBACTION: " + action);
